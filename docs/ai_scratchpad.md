...
- SOGP learner UI should combine public-site typography, tokens, warmth, and cohesion with modern MOOC density: on mobile show a compact Monday–Sunday current-week calendar above content with an explicit full-calendar disclosure; on desktop use left course navigation, central teaching/Prayer Watch/assessment activities, and a right progress/review/community rail. Keep the four level-status cards on the main page, separate each required activity clearly, and avoid oversized teaching titles. Do not copy `/ppc` zinc styling or redesign tokens.
