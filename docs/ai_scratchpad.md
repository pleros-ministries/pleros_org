...
- PPC DB routes need `DATABASE_URL`; use `DATABASE_URL_UNPOOLED` with `Pool`/`Client` for bulk content scripts.
...
- Pre-SOGP is a gated 30-day consecutive calendar ending before cohort start. Each day requires a manually completed lesson and Morning Prayer Watch; past incomplete is red/recoverable, current/future incomplete grey, complete green. Calendar precedes content on mobile.
...
- Learner UI combines public typography/warmth with MOOC density. Mobile: compact current-week calendar and a real left course drawer. Desktop: fixed left progress/outline rail, central activities, right progress/review rail.
- Keep teaching titles restrained, metadata muted, and activity panels neutral, spaced, independently collapsible, with simple grey headers/dividers. Reserve colour for state and primary actions; omit a Telegram promo card from the context rail.
...
- The landing page has no navbar: white hero, four-part headline, no pre-headline label, `What are you seeking?`, natural-height question rows, welcome video, then `What is SOGP?`. Outcomes live there; only the footer links away.
