...
- Certificate gating requires all 24 assessments, at least 80% Morning Prayer Watch attendance across 28 cohort dates, and at least 80% attendance of the required daily Telegram review sessions (`requiredLiveClassPercent`, one per cohort day, both weekday and Sunday). Podcast is independent. Daily review sessions seeded before a cohort's "today" stay optional (`isRequired: false`, grandfathered in) so the policy change isn't retroactive; sessions from today onward seed as required.
...
- Telegram bot broadcasts only; never post learner PII or promise discussion until a group is actually linked. Use admin composer for immediate sends. Exception: completed-enrolment alerts (name, phone, country, state, year of birth, referral) post to the separate private ops chat `TELEGRAM_SOGP_SIGNUP_CHAT_ID` via `lib/telegram/sogp-signup-alert.ts`, fired fire-and-forget from `app/api/sogp/enrol/complete/route.ts`; never send learner PII to the public `TELEGRAM_SOGP_CHANNEL_ID` channel.
...
- The main homepage uses a dedicated one-time SOGP promotional sheet with no lead form; its CTA leads to `/sogp`, where the landing page continues into `/sogp/enrol`, and its dismissal state is independent of Welcome Pack access. `/welcome` is retired and permanently redirects to `/sogp`.
...
- `/sogp/enrol` has no navbar. Required fields: first name, surname, E.164 phone, editable country, state/province/region, year of birth, referral source, and explicit initially-unselected WhatsApp Yes/No consent. Use inline native radios for WhatsApp consent with 12px separation from the legend. Render referral sources through the existing Base UI popup pattern so the compact list is visible and keyboard-accessible. List named social platforms first—Facebook, Instagram, TikTok, WhatsApp, Twitter, and Telegram—then personal, church, Pleros, search, and Other sources; do not use a generic `Social media` option. Do not restore the removed outcome question.
