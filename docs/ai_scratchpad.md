...
- Public landing copy source is the Google Doc titled `SOGP Landing Page`; adopt all substantive visitor-facing copy verbatim, preserving wording, capitalization, punctuation, and current typos. Do not summarize, polish, reorder, or invent replacement prose. Treat `Headline/Subheadline`, `Visual`, and `Video` as production instructions, not visitor copy; omit empty facilitator, social-proof, and FAQ sections until body copy exists.
