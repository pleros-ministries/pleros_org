...
- Public landing copy source is the Google Doc titled `SOGP Landing Page`; preserve its substantive wording and order, but correct obvious grammar, spacing, punctuation, and typos. Do not summarize, broadly rewrite, or invent replacement prose. Treat `Headline/Subheadline`, `Visual`, and `Video` as production instructions, not visitor copy. Use the existing About-page Senior Minister profile for SOGP’s `Lead Facilitator`; omit empty social-proof and FAQ sections until body copy exists.
...
- SOGP landing curriculum is an editorial title-only list grouped into independently collapsible levels: Level 1 open initially, later levels closed. Keep continuous numbering, remove repeated per-track level badges and aggregate “confirmed tracks” copy, and add Level 3 only after explicit selection. Structure gets its own distinct treatment below.
