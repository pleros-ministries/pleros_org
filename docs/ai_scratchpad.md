...
- Pre-SOGP is a gated 30-day consecutive calendar ending before cohort start. Match the SOGP dashboard shell with a sticky dark-blue nav/back action, lime PRE-SOGP identifier, compact lessons/countdown row, current-day indicator, lesson title inside a collapsible Preparation lesson activity, and desktop sticky offset below the nav. Each day requires a manually completed lesson and Morning Prayer Watch; past incomplete is red/recoverable, current/future incomplete grey, complete green. Calendar precedes content on mobile.
