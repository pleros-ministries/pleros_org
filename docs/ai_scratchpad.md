...
- Telegram provides launch community, text, and voice notes. After DB persistence, both the SOGP enrolment redirect and confirmation email point directly to `/dashboard/welcomepack/join`; that introduction page retains the configured Telegram CTA. The email sender name is `Pleros Ministries & Missions`. Do not use bot deep links or identity-linking in this flow. Bot handles scheduled/admin-triggered channel broadcasts only; never post learner PII. Do not build native discussion or voice storage for launch.
- SOGP enrolment email uses soft sky/white surfaces and progressively enhances to the public Sen/Be Vietnam Pro pairing while remaining layout-safe in Arial/Helvetica; keep CTA weight at 500, use one concise dashboard instruction followed by `Open Dashboard`, omit decorative pill emoji, and protect classic Outlook with an explicit MSO fallback.
...
- Keep the SOGP enrolment hero to two balanced lines on standard mobile widths, and use familiar outline symbols in its summary list (`CircleCheck` for free enrolment, not decorative sparkles).
...
- The SOGP landing welcome video uses a 1:1 versioned self-hosted H.264/AAC MP4 with a matching square poster and native player; keep the separate Welcome Pack orientation media unchanged, avoid YouTube branding, and cache each video/poster pair as immutable Vercel CDN assets.
- SOGP landing FAQs belong after Benefits in a mobile-first pastel-blue/white accordion. Approved basics cover free cost, participation from another church, eligibility, an originally four-week but pressure-free self-paced format, device/platform needs, and certification demands deferred to the orientation message rather than listed on the landing page.
