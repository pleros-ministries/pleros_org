...
- Learner UI combines public typography/warmth with MOOC density. Use a compact sticky SOGP app nav with the Dashboard back action, then keep the greeting/cohort in one responsive content row below it. Mobile: keep the compact current-week calendar above activities and omit the course-menu trigger/drawer. Desktop: fixed left progress/outline rail, central activities, right progress/review rail, with sticky offsets clearing the nav.
