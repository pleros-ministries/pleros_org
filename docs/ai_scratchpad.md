...
- Telegram bot broadcasts only; never post learner PII or promise discussion until a group is actually linked. Use admin composer for immediate sends. Exception: completed-enrolment alerts (name, phone, country, state, age, referral) post to the separate private ops chat `TELEGRAM_SOGP_SIGNUP_CHAT_ID` via `lib/telegram/sogp-signup-alert.ts`, fired fire-and-forget from `app/api/sogp/enrol/complete/route.ts`; never send learner PII to the public `TELEGRAM_SOGP_CHANNEL_ID` channel.
