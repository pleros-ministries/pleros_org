...
- SOGP learner UI should combine public-site typography, tokens, warmth, and cohesion with modern MOOC density: on mobile show a compact Monday–Sunday current-week calendar above content with an explicit full-calendar disclosure; on desktop use left course navigation, central teaching/Prayer Watch/assessment activities, and a right progress/review rail. Keep the four level-status cards on the main page and avoid oversized teaching titles. Activity panels should be neutral white, generously separated, independently collapsible, and use a divider under each header; reserve strong colour for small state cues and primary actions. Do not add a Telegram/community promotion card to the context rail, copy `/ppc` zinc styling, or redesign tokens.
