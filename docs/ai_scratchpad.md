...
- Dashboard church ministry invitations should sit as a full-width strip above the footer, use a non-footer light background with the church logo as a visible bottom-corner masked/gradient watermark rather than a faded opacity image, use `color-text-strong` for heading and body copy on the tinted blue surface, keep generous internal rhythm between eyebrow/copy/CTA, and include a `Learn more` CTA to `/fcc` rather than crowding the Start Here card grid.
