...
- SOGP learner UI should combine public-site typography, tokens, warmth, and cohesion with modern MOOC density: on mobile show a compact Monday–Sunday current-week calendar above content with an explicit full-calendar disclosure; on desktop use left course navigation, central teaching/Prayer Watch/assessment activities, and a right progress/review rail. Keep the four level-status cards on the main page, separate each required activity clearly, avoid oversized teaching titles, and do not add a Telegram/community promotion card to the context rail. Do not copy `/ppc` zinc styling or redesign tokens.
