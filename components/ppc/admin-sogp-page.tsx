...
import { AdminSogpReport } from "@/components/ppc/admin-sogp-report";
import { AdminSogpPreparation } from "@/components/ppc/admin-sogp-preparation";
...
const tabs = ["Overview", "Cohorts", "Curriculum", "Preparation", "Enrolments", "Orientation Survey", "Live classes", "Completion", "Report"] as const;
...
export function Metric({ label, value, hint, Icon }: { label: string; value: string | number; hint: string; Icon: typeof Users }) {
...
function LiveClassLinks({ item, onSaved }: { item: { id: number; youtubeLiveUrl: string | null; recordingUrl: string | null }; onSaved: () => Promise<unknown> }) {
  const [editing, setEditing] = useState(false);
  const [youtubeUrl, setYoutubeUrl] = useState(item.youtubeLiveUrl ?? "");
  const [recordingUrl, setRecordingUrl] = useState(item.recordingUrl ?? "");
  const mutation = useMutation({
    mutationFn: async () => {
      const result = await updateSogpLiveClass({ id: item.id, youtubeLiveUrl: youtubeUrl.trim() || null, recordingUrl: recordingUrl.trim() || null });
      if (result.error) throw new Error(result.error);
      return result;
    },
    async onSuccess() { setEditing(false); await onSaved(); },
  });
  if (!editing) return <button type="button" onClick={() => setEditing(true)} className="w-fit text-[10px] font-medium text-[var(--color-brand-blue)] underline underline-offset-2">Edit links</button>;
  return <div className="grid gap-2 rounded-sm border border-zinc-200 bg-zinc-50 p-2">
    <label className="grid gap-1 text-[10px] text-zinc-600">YouTube live URL<input value={youtubeUrl} onChange={(event) => setYoutubeUrl(event.target.value)} type="url" placeholder="https://youtube.com/…" className="h-7 rounded-sm border border-zinc-200 px-2 text-xs"/></label>
    <label className="grid gap-1 text-[10px] text-zinc-600">Recording URL<input value={recordingUrl} onChange={(event) => setRecordingUrl(event.target.value)} type="url" placeholder="https://…" className="h-7 rounded-sm border border-zinc-200 px-2 text-xs"/></label>
    {mutation.error ? <p className="text-[10px] text-rose-700">{mutation.error.message}</p> : null}
    <div className="flex gap-2">
      <button type="button" disabled={mutation.isPending} onClick={() => mutation.mutate()} className="h-7 rounded-sm bg-[var(--color-brand-blue)] px-2 text-[10px] font-medium text-white disabled:opacity-50">{mutation.isPending ? "Saving" : "Save"}</button>
      <button type="button" onClick={() => { setEditing(false); setYoutubeUrl(item.youtubeLiveUrl ?? ""); setRecordingUrl(item.recordingUrl ?? ""); }} className="h-7 rounded-sm border border-zinc-200 px-2 text-[10px] font-medium text-zinc-600">Cancel</button>
    </div>
  </div>;
}
...
export function AdminSogpPage() {
  ...
  {tab==="Curriculum"?<section className="grid gap-4 lg:grid-cols-[1fr_24rem]">
    <div className="overflow-hidden rounded-sm border border-zinc-200 bg-white">
      <div className="border-b border-zinc-100 px-4 py-3"><h2 className="ppc-heading text-sm font-semibold">SOGP curriculum</h2><p className="mt-0.5 text-xs text-zinc-500">24 required teachings across four levels.</p></div>
      <div className="divide-y divide-zinc-100">{currentTracks.map((track)=><div key={track.id} className="grid grid-cols-[4.5rem_1fr_auto] items-center gap-3 px-4 py-3 text-xs"><span className="font-medium text-zinc-500">Track {track.curriculumOrder}</span><span className="text-zinc-900">SOGP L{track.curriculumLevel} · {track.title}</span><span className={track.ready?"text-emerald-700":"text-amber-700"}>{track.ready?"Ready":"Missing"}</span></div>)}</div>
    </div>
  ...
  {tab==="Report"?<AdminSogpReport tracks={data.tracks} liveClasses={data.liveClasses}/>:null}
  </div>;
}
